...
import liam.myths.dimension.egypt.*;
...
public class BlockTutorialPortal extends BlockBreakable
{
	...
	{
		super(par1, "mythologymod:egyptPortal", Material.portal, false);
		this.setTickRandomly(true);
		this.setHardness(-1.0F);
		this.setStepSound(soundGlassFootstep);
		this.setLightValue(0.75F);
		this.setCreativeTab(CreativeTabs.tabBlock);
		this.func_111022_d("mythologymod:egyptPortal");
	}
	/**
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		...
		else
		{
			...
			for (l = -1; l <= 2; ++l)
			{
				for (i1 = -1; i1 <= 3; ++i1)
				{
					...
					if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
					{
						...
						else if (j1 != 0 && j1 != Myths.TutorialFire.blockID)
						{
							return false;
						}
					}
			...
			for (l = 0; l < 2; ++l)
			{
				for (i1 = 0; i1 < 3; ++i1)
				{
					par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, Myths.TutorialPortal.blockID, 0, 2);
				}
			}
			return true;
		}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
	{
		if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
		{
			...
			else if (thePlayer.dimension != Myths.DimID)
			{
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, Myths.DimID, new EgyptTeleporter(thePlayer.mcServer.worldServerForDimension(Myths.DimID)));
			}
			else {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new EgyptTeleporter(thePlayer.mcServer.worldServerForDimension(0)));
			}
		}
	}
	@SideOnly(Side.CLIENT)
